...
func _ready():
	...
	if multiplayer.get_unique_id() != 1:
		$BeanSprite.flip_h = true
		$Gun/GunSprite.flip_h = true
		$Gun/GunSprite.position.x = -30
	
	$Label.text = GameManager.Players[$MultiplayerSynchronizer.get_multiplayer_authority()].name
...
func _input(_event):
	if $MultiplayerSynchronizer.get_multiplayer_authority() == multiplayer.get_unique_id():
		...
		if Input.is_action_pressed(input_right):
			acc.x = max_acceleration
			$BeanSprite.play("walk")
			$BeanSprite.flip_h = false
			$Gun/GunSprite.flip_h = false
			$Gun/GunSprite.position.x = 0
...
func _physics_process(delta : float):
	...
	move_and_slide()
	if velocity.x != 0 and is_feet_on_ground():
		$BeanSprite.play("walk")
	if (velocity.x < 22 and velocity.x > 0) or (velocity.x > -22 and velocity.x < 0) or (not is_feet_on_ground()):
		$BeanSprite.play("still")
